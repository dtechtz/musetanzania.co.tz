       ...
       String  enddate = request.getParameter("enddate");
        ...
        try {
           ...
           // pst.setString(1, Name);
            rs = pst.executeQuery();
             %>
<p style="color: #006666; padding: 1rem; font-weight: bold; background: #f0f0f0" > GROUP  MEMBER LOAN COLLECTED<span style="float: right"> <a href="PDF.jsp?Report=groupcollection&startdate=<%=startdate%>&enddate=<%=enddate%>" target="_black" class="btn btn-success"><span class="fa fa-download">&nbsp;&nbsp;Generate Printable Report</span></a></span></p>
               <table id="table4" class="table">
                                                <thead>
                                                    <tr>
                                                        <th style="width: 3%">#</th>
                                                        <th  style="width: 8%">MemberID</th>
                                                        <th  style="width: 10%">GroupID:</th>
...
<%
        ...
        try {
             if(con.isClosed()){
              ...
              pst = con.prepareStatement("SELECT ( members.ID) AS MemberID,members.Reg_ID,members.Fname,members.Mname,members.Lname,members.Phone,members.Group_name, loan_group.ID, (loan_group.Name) AS LoanGroup, loan_return.UserID, loan_return.Invoice, loan_return.Return_Amount, loan_return.Date from members  INNER JOIN  loan_group ON members.Group_name = loan_group.ID INNER JOIN  loan_return ON  loan_return.UserID = members.ID  WHERE  loan_return.Date BETWEEN '"+startdate+"' AND '"+enddate+"' ");
           // pst.setString(1, Name);
            rs = pst.executeQuery();
             %>
<p style="color: #006666; padding: 1rem; font-weight: bold; background: #f0f0f0" > INDIVIDUAL MEMBER LOAN  COLLECTED<span style="float: right"> <a href="PDF.jsp?Report=privatecollection&startdate=<%=startdate%>&enddate=<%=enddate%>" target="_black" class="btn btn-success"><span class="fa fa-download">&nbsp;&nbsp;Generate Printable Report</span></a></span></p>
